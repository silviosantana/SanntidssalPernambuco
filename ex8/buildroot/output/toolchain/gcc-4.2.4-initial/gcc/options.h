...
extern int avr32_imm_in_const_pool;
extern const char *avr32_part_name;
extern int linux_uclibc;
extern int profile_flag;
extern int pedantic;
...
#define MASK_HAS_ASM_ADDR_PSEUDOS (1 << 0)
#define MASK_COND_EXEC_BEFORE_RELOAD (1 << 1)
#define MASK_FAST_FLOAT (1 << 2)
#define MASK_FORCE_DOUBLE_ALIGN (1 << 3)
#define MASK_HARD_FLOAT (1 << 4)
#define MASK_MD_REORG_OPTIMIZATION (1 << 5)
#define MASK_NO_INIT_GOT (1 << 6)
#define MASK_NO_PIC (1 << 7)
#define MASK_RELAX (1 << 8)
#define OPTION_MASK_UCLIBC (1 << 0)
#define MASK_USE_RODATA_SECTION (1 << 9)

#define TARGET_HAS_ASM_ADDR_PSEUDOS ((target_flags & MASK_HAS_ASM_ADDR_PSEUDOS) != 0)
#define TARGET_COND_EXEC_BEFORE_RELOAD ((target_flags & MASK_COND_EXEC_BEFORE_RELOAD) != 0)
#define TARGET_FAST_FLOAT ((target_flags & MASK_FAST_FLOAT) != 0)
#define TARGET_FORCE_DOUBLE_ALIGN ((target_flags & MASK_FORCE_DOUBLE_ALIGN) != 0)
#define TARGET_HARD_FLOAT ((target_flags & MASK_HARD_FLOAT) != 0)
#define TARGET_MD_REORG_OPTIMIZATION ((target_flags & MASK_MD_REORG_OPTIMIZATION) != 0)
#define TARGET_NO_INIT_GOT ((target_flags & MASK_NO_INIT_GOT) != 0)
#define TARGET_NO_PIC ((target_flags & MASK_NO_PIC) != 0)
#define TARGET_RELAX ((target_flags & MASK_RELAX) != 0)
#define OPTION_UCLIBC ((linux_uclibc & OPTION_MASK_UCLIBC) != 0)
#define TARGET_USE_RODATA_SECTION ((target_flags & MASK_USE_RODATA_SECTION) != 0)
...
enum opt_code
{
  ...
  OPT_march_,                                /* -march= */
  OPT_masm_addr_pseudos,                     /* -masm-addr-pseudos */
  OPT_mcond_exec_before_reload,              /* -mcond-exec-before-reload */
  OPT_mcpu_,                                 /* -mcpu= */
  OPT_mfast_float,                           /* -mfast-float */
  OPT_mforce_double_align,                   /* -mforce-double-align */
  OPT_mglibc,                                /* -mglibc */
  OPT_mhard_float,                           /* -mhard-float */
  OPT_mimm_in_const_pool,                    /* -mimm-in-const-pool */
  OPT_mmd_reorg_opt,                         /* -mmd-reorg-opt */
  OPT_mno_init_got,                          /* -mno-init-got */
  OPT_mno_pic,                               /* -mno-pic */
  OPT_mpart_,                                /* -mpart= */
  OPT_mrelax,                                /* -mrelax */
  OPT_msoft_float,                           /* -msoft-float */
  OPT_muclibc,                               /* -muclibc */
  OPT_muse_rodata_section,                   /* -muse-rodata-section */
