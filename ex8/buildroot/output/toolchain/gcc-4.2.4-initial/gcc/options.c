...
const char *avr32_arch_name;

/* Set by -mcpu=.
   Specify the AVR32 part name (deprecated)  */
const char *avr32_part_name;
...
const struct cl_option cl_options[] =
{
  ...
  { "-mcond-exec-before-reload",
    ...
    &target_flags, CLVC_BIT_SET, MASK_COND_EXEC_BEFORE_RELOAD },
  { "-mcpu=",
    "Specify the AVR32 part name (deprecated)",
    N_OPTS, 5, -1,
    CL_TARGET | CL_JOINED | CL_REJECT_NEGATIVE | CL_UNDOCUMENTED | CL_REPORT,
    &avr32_part_name, CLVC_STRING, 0 },
  { "-mfast-float",
    "Enable fast floating-point library. Enabled by default if the -funsafe-math-optimizations switch is specified.",
    N_OPTS, 11, 543,
    CL_TARGET | CL_REPORT,
    &target_flags, CLVC_BIT_SET, MASK_FAST_FLOAT },
  { "-mforce-double-align",
    "Force double-word alignment for double-word memory accesses.",
    N_OPTS, 19, -1,
    CL_TARGET | CL_REJECT_NEGATIVE | CL_REPORT,
    &target_flags, CLVC_BIT_SET, MASK_FORCE_DOUBLE_ALIGN },
  { "-mglibc",
    "Use GNU libc instead of uClibc",
    N_OPTS, 6, -1,
    CL_TARGET | CL_REJECT_NEGATIVE | CL_REPORT,
  ...
  { "-mhard-float",
    ...
    N_OPTS, 11, 546,
    CL_TARGET | CL_UNDOCUMENTED | CL_REPORT,
    &target_flags, CLVC_BIT_SET, MASK_HARD_FLOAT },
  { "-mimm-in-const-pool",
    "Put large immediates in constant pool. This is enabled by default for archs with insn-cache.",
    N_OPTS, 18, 547,
    CL_TARGET | CL_REPORT,
    &avr32_imm_in_const_pool, CLVC_BOOLEAN, 0 },
  { "-mmd-reorg-opt",
    "Perform machine dependent optimizations in reorg stage.",
    N_OPTS, 13, 548,
    CL_TARGET | CL_UNDOCUMENTED | CL_REPORT,
    &target_flags, CLVC_BIT_SET, MASK_MD_REORG_OPTIMIZATION },
  { "-mno-init-got",
    "Do not initialize GOT register before using it when compiling PIC code.",
    N_OPTS, 12, -1,
    CL_TARGET | CL_REJECT_NEGATIVE | CL_REPORT,
    &target_flags, CLVC_BIT_SET, MASK_NO_INIT_GOT },
  { "-mno-pic",
    "Do not generate position-independent code. (deprecated, use -fno-pic instead)",
    N_OPTS, 7, -1,
    CL_TARGET | CL_REJECT_NEGATIVE | CL_REPORT,
    &target_flags, CLVC_BIT_SET, MASK_NO_PIC },
  { "-mpart=",
  ...
  { "-mrelax",
    ...
    N_OPTS, 6, 552,
    CL_TARGET | CL_REPORT,
    &target_flags, CLVC_BIT_SET, MASK_RELAX },
  { "-msoft-float",
    "Use software floating-point library for floating-point operations.",
    N_OPTS, 11, 553,
    CL_TARGET | CL_UNDOCUMENTED | CL_REPORT,
    &target_flags, CLVC_BIT_CLEAR, MASK_HARD_FLOAT },
  { "-muclibc",
    "Use uClibc instead of GNU libc",
    N_OPTS, 7, -1,
    CL_TARGET | CL_REJECT_NEGATIVE | CL_REPORT,
  ...
  { "-muse-rodata-section",
    ...
    N_OPTS, 19, 555,
    CL_TARGET | CL_REPORT,
    &target_flags, CLVC_BIT_SET, MASK_USE_RODATA_SECTION },
  { "-nostdinc",
