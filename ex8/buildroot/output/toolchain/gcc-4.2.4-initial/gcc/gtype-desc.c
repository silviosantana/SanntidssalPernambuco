...
gt_ggc_mx_machine_function (void *x_p)
{
  ...
  if (ggc_test_and_set_mark (x))
    {
      gt_ggc_m_15minipool_labels ((*x).minipool_label_head);
      gt_ggc_m_15minipool_labels ((*x).minipool_label_tail);
    }
}

void
gt_ggc_mx_minipool_labels (void *x_p)
{
  struct minipool_labels * x = (struct minipool_labels *)x_p;
  struct minipool_labels * xlimit = x;
  while (ggc_test_and_set_mark (xlimit))
   xlimit = ((*xlimit).next);
  if (x != xlimit)
    for (;;)
      {
        struct minipool_labels * const xprev = ((*x).prev);
        if (xprev == NULL) break;
        x = xprev;
        (void) ggc_test_and_set_mark (xprev);
      }
  while (x != xlimit)
    {
      ...
      gt_ggc_m_15minipool_labels ((*x).prev);
      gt_ggc_m_15minipool_labels ((*x).next);
      x = ((*x).next);
    }
}
...
gt_pch_nx_machine_function (void *x_p)
{
  ...
  if (gt_pch_note_object (x, x, gt_pch_p_16machine_function, gt_ggc_e_16machine_function))
    {
      gt_pch_n_15minipool_labels ((*x).minipool_label_head);
      gt_pch_n_15minipool_labels ((*x).minipool_label_tail);
    }
}

void
gt_pch_nx_minipool_labels (void *x_p)
{
  struct minipool_labels * x = (struct minipool_labels *)x_p;
  struct minipool_labels * xlimit = x;
  while (gt_pch_note_object (xlimit, xlimit, gt_pch_p_15minipool_labels, gt_ggc_e_15minipool_labels))
   xlimit = ((*xlimit).next);
  if (x != xlimit)
    for (;;)
      {
        ...
        if (xprev == NULL) break;
        x = xprev;
        (void) gt_pch_note_object (xprev, xprev, gt_pch_p_15minipool_labels, gt_ggc_e_15minipool_labels);
      }
  while (x != xlimit)
    {
      gt_pch_n_7rtx_def ((*x).label);
      gt_pch_n_15minipool_labels ((*x).prev);
      gt_pch_n_15minipool_labels ((*x).next);
      x = ((*x).next);
    }
}
	...
	ATTRIBUTE_UNUSED void *cookie)
{
  ...
  if ((void *)(x) == this_obj)
    op (&((*x).minipool_label_head), cookie);
  if ((void *)(x) == this_obj)
    op (&((*x).minipool_label_tail), cookie);
}

void
gt_pch_p_15minipool_labels (ATTRIBUTE_UNUSED void *this_obj,
	void *x_p,
	ATTRIBUTE_UNUSED gt_pointer_operator op,
	ATTRIBUTE_UNUSED void *cookie)
{
  struct minipool_labels * const x ATTRIBUTE_UNUSED = (struct minipool_labels *)x_p;
  if ((void *)(x) == this_obj)
    op (&((*x).label), cookie);
  if ((void *)(x) == this_obj)
    op (&((*x).prev), cookie);
